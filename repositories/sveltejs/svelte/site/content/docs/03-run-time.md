...
Модуль `svelte/store` предоставляет функции для создания [хранилищ](docs#4_Dobavte_prefiks_$_k_hranilishhu_dlya_polucheniya_ego_znacheniya).
...
Параметр `hydrate` инструктирует Svelte обновить существующий DOM (обычно полученный в ходе рендеринга на стороне сервера), а не создавать новые элементы. Это будет работать только в том случае, если компонент был скомпилирован с параметром [`hydratable: true`](docs#svelte_compile).
...
Компоненты Svelte также могут быть скомпилированы в пользовательские элементы (или web-компоненты) с помощью параметра компилятора `customElements: true`. Вы должны указать имя тега для компонента, используя [элемент](docs#svelte_options) `<svelte:options>`.
