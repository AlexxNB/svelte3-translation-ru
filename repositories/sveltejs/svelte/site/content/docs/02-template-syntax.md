...
Значения из числовых `input` элементов приводятся к числу. Хоть значение свойства `input.value` и является строкой для DOM, Svelte будет рассматривать его как число. Если значение `input` пустое или недействительное (в случае `type="number"`), оно будет равно `undefined`.
...
Переход инициируется добавлением или удалением элемента из DOM в результате изменения состояния приложения. Переходы запускаются не при первом монтировании компонента, а только при последующих обновлениях.
...
Локальные переходы воспроизводятся только когда создается или убирается конкретный блок, к которому они прикреплены, а *не* их родительские блоки.
