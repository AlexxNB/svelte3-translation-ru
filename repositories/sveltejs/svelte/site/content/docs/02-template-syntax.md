...
Значения из числовых `input` элементов приводятся к числу. Хоть значение свойства `input.value` и является строкой для DOM, но Svelte будет рассматривать его как число. Если значение `input` пустое или недействительное (в случае `type="number"`), оно будет равно `undefined`.
...
> Не беспокойтесь о том факте, что мы объявляем функцию `foo` в каждом экземпляре компонента - Svelte выведет из контекста компонента любые функции, которые не зависят от его локального состояния.
...
Переход инициируется добавлением или удалением элемента из DOM в результате изменения состояния приложения. Переходы не запускаются при первом монтировании компонента, а только при последующих обновлениях.

Элементы внутри *исчезающего* блока будут храниться в DOM до тех пор, пока не будут выполнены все запущенные переходы.

Директива `transition:` указывает на то, что переход является *обратимым*. Т.е. он может в любой момент начать проигрываться в другую сторону.
...
Переходы могут использовать пользовательские функции. Если возвращенный объект имеет функцию `css`, Svelte создаст CSS анимацию, которая воспроизводится на элементе.
...
Если переход возвращает функцию вместо объекта перехода, то она будет вызвана в следующей микрозадаче. Это позволяет координировать несколько переходов, что дает возможность запускать [перекрестные переходы](https://v3.svelte.technology/tutorial/deferred-transitions).
...
Локальные переходы воспроизводятся только, когда создается или убирается конкретный блок, к которому они прикреплены, а *не* родительские блоки.
...
Вы также можете сделать привязку к следующим свойствам:
