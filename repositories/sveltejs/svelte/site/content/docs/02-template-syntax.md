...
Начиная с [Svelte 3.38](https://github.com/sveltejs/svelte/issues/6268) ([RFC](https://github.com/sveltejs/rfcs/pull/13)), вы можете передавать стили в качестве реквизита компонентам для целей тематизации, используя пользовательские свойства CSS.
...
**Примечание**: Поскольку это дополнительный div, помните, что ваша структура CSS может случайно зацепиться за это. Помните об этом добавленом элементе обертки при использовании этой функции. Также обратите внимание, что не все браузеры поддерживают `display: contents`: https://caniuse.com/css-display-contents
...
Подобно `<svelte:window>`, этот элемент позволяет добавлять обработчики событий на `document.body`, такие как `mouseenter` и `mouseleave`, которые не запускаются на `window`. Он также позволяет использовать [actions](docs#use_action) для элемента `<body>`.
...
Как и в случае со `<svelte:window` и `<svelte:head`, этот элемент должен быть на верхнем уровне вашего компонента и не может находиться внутри блока или другого элемента.
