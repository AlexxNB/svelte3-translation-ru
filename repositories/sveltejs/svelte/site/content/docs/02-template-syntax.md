...
Он не может отображаться на верхнем уровне разметки, а должен быть помещён внутри блока `if` или `each` или передан в слот компонента, чтобы избежать бесконечного цикла.
