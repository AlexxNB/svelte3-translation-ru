...
Как и в случае с `<svelte:window>`, этот элемент позволяет добавлять обработчики событий к `document.body`, например к `mouseenter` и`mouseleave`, которые не запускаются в `window`; и он должен быть на верхнем уровне вашего компонента.
