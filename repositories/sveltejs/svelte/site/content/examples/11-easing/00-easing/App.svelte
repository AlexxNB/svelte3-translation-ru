<script>
	...
	async function runAnimations() {
		...
		await Promise.all([time.set(1000, { duration, easing: (x) => x }), value.set(0, { duration, easing: current.fn })]);
...
<div bind:offsetWidth={width} class="easing-vis">
	...
	<Controls {eases} {types} {playing} {width} bind:duration bind:current_ease bind:current_type on:play={runAnimations} />
</div>
